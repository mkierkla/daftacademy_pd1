...
async def get_sales(category: str = Query(None)):
	...
	if category == 'genres':
		app.db_connection.row_factory = sqlite3.Row
		get_data = app.db_connection.execute(
			"SELECT (SELECT genres.Name FROM genres WHERE tracks.GenreId=genres.GenreId) as Name, SUM(invoice_items.UnitPrice*invoice_items.Quantity) as Sum FROM tracks INNER JOIN invoice_items ON tracks.TrackId = invoice_items.TrackId GROUP BY tracks.GenreId ORDER BY Sum DESC"
			).fetchall()
		return get_data
